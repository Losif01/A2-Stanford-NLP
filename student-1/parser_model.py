...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(n_features*embeddings.shape[1], hidden_size)))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.empty(hidden_size)))
        ###     2) Construct `self.dropout` layer.
        self.dropout= nn.Dropout(dropout_prob) #dropout takes a probability as a parameter (0 to 1)
        ###     3) Declare `self.hidden_to_logits_weight` and `self.hidden_to_logits_bias` as `nn.Parameter`.
        ###        Initialize weight with the `nn.init.xavier_uniform_` function and bias with `nn.init.uniform_`
        ###        with default parameters.
        self.hidden_to_logits_weight = nn.Parameter(hidden_size, n_classes)
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.empty(n_classes)))
    ...
    def embedding_lookup(self, w):
        ...
        ### TODO:
        ###     1) For each index `i` in `w`, select `i`th vector from self.
        x = None
        x = self.embeddings[w]
